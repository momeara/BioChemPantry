...
protein_class_ids <- pantry |>
	...
	dplyr::collect(n = Inf) |>
	dplyr::mutate(
	  protein_class_id = as.integer(protein_class_id),
...
parents <- parents |>
	...
	unlist() |>
	matrix(ncol=max_depth + 1, byrow = TRUE) |>
	as.data.frame()

colnames(parents) <- c(
  "protein_class_id",
  paste("class_", 1:max_depth, sep = ""))
...
protein_class_ids <- protein_class_ids |>
	...
	reshape2::dcast(
	  protein_class_id + short_name ~ parent_class_level,
...
chembl_target_classes <-
  ...
  dplyr::tbl("component_sequences") |>
	...
	dplyr::left_join(
		pantry |> dplyr::tbl("component_class") |>
		dplyr::select(
			component_id,
			protein_class_id),
		by = "component_id") |>
...
if (!dir.exists(paste0(staging_directory, "/data"))) {
  cat(
    "Creating directory: ",
    "'", paste0(staging_directory, "/data"), "'\n", sep = "")
