...
pantry <- BioChemPantry::get_pantry("chembl25")
...
#	ensembl_id_alt = `Cross-reference (Ensembl)`,
	...
	dplyr::mutate(
		entrez_id = entrez_id_alt %>% stringr::str_extract("^[0-9]+"))

uniprot_targets %>%
	readr::write_tsv(paste0(staging_directory, "/data/uniprot_targets_raw_170814.tsv"))
uniprot_targets <- readr::read_tsv(
	paste0(staging_directory, "/data/uniprot_targets_raw_170814.tsv"))
...
target_classes <- read_tsv(paste0(staging_directory, "/data/chembl_target_classes.tsv")) %>%
	dplyr::select(uniprot_accn, class_1, class_2) %>%
